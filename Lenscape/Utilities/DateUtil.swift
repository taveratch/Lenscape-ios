...
class DateUtil {
    ...
    static func getShortenRelativeString(since sinceMillisecond: Double, until untilMillisecond: Double = Double(Date().timeIntervalSince1970*1000)) -> String {
        let sinceDate = Date(timeIntervalSince1970: sinceMillisecond/1000.0)
        let untilDate = Date(timeIntervalSince1970: untilMillisecond/1000.0)
        let diff = (Double((untilDate.timeIntervalSince1970).rounded()) - Double((sinceDate.timeIntervalSince1970).rounded()))
        ...
        if diff < hour {
            ...
            return "\(Int(n)) minute\(isPlural(n: n) ? "s" : "") ago"
        } else if diff < hours_24 {
            let n = ceil(diff/hour)
            return "\(Int(n)) hour\(isPlural(n: n) ? "s" : "") ago"
        } else if diff < days_2 {
            let n = ceil(diff/hours_24)
            return "\(Int(n)) day\(isPlural(n: n) ? "s" : "") ago"
        } else {
            return "\(dateString) \(timeString)"
        }
    }
